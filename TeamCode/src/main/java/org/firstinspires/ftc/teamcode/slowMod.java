...
public class slowMod {
    ...
    public void drive_manager(DcMotor leftFront, DcMotor leftBack, DcMotor rightFront, DcMotor rightBack, Gamepad gamepad1)
    {
        ...
        {
            forward(leftFront, leftBack, rightFront, rightBack, gamepad1.right_trigger/3);
        }
        else if (gamepad1.left_trigger > 0)
        {
            backward(leftFront, leftBack, rightFront, rightBack, gamepad1.left_trigger/3);
        }
        else if(gamepad1.left_stick_x > 0)
        {
            turnRight(leftFront, leftBack, rightFront, rightBack, -gamepad1.left_stick_x/3);
        }
        else if(gamepad1.left_stick_x < 0)
        {
            turnLeft(leftFront, leftBack, rightFront, rightBack, gamepad1.left_stick_x/3);
        }
        else if (gamepad1.dpad_right)
        {
            driveRight(leftFront, leftBack, rightFront, rightBack, 0.25);
        }
        else if (gamepad1.dpad_left)
        {
            driveLeft(leftFront, leftBack, rightFront, rightBack, 0.25);
        }
        else
        {
